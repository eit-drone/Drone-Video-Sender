...
def make_timing_data(frame_number: int, frame, print_func = print) -> str:
    ...
    print_func(f"Frame {frame_number} encoded to {len(extended)} bytes")
    return extended


def parse_timing_frame(payload: bytes, print_func = print) -> str:
    ...
    print_func(f"Frame {frame_number} received at {timestamp} with {len(jpg_as_packed)} bytes (Delay: {datetime.datetime.now() - datetime.datetime.fromisoformat(timestamp)}))")
